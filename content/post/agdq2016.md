...
La route consiste donc à collecter les objets pour pouvoir construire un feu de camp, de récupérer le Sniper Rifle et de se téléporter à la salle du boss.

J'ai réalisé un TAS test de cette route avec la version originale du jeu :
...
Les participants de la compétition de TAS pouvaient soumettre leur TAS via un formulaite disponible sur une iframe du site TASVideos. Le système de soumission a été récupéré de l'édition précédente. Il est composé d'un site créé avec [Django](https://www.djangoproject.com/) et tournait sur un serveur que dwangoAC avait à disposition. Le site vérifiait quelques informations sur le TAS soumis, comme le contenu du champ `auteur` qui était utilisé pour identifier l'auteur pour la compétition, ainsi que la somme de contrôle pour vérifier que le TAS a été fait sur la bonne version du jeu.

# Juge

Le système de juge des soumissions était automatisé. Il a aussi été récupéré de l'édition précédente, avec quelques ajouts.